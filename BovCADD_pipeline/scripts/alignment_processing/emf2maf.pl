...
# Modified to work on gzipped files instead.
# By: Job van Schipstal
# Date: 5-9-2023
...
my $VERSION = "1.0.1.MODIFIED";

if (@ARGV < 2) {
    print STDERR qq"
  emf2maf.pl v$VERSION - EMF to MAF file converter.
  Use: emf2maf.pl <infile.emf.gz> <outfile.maf.gz>
  The script was modified to work on gzipped files.
";
    exit 1;
}
...
open(my $emf_fh, '-|', '/usr/bin/env', 'gzip', '-d', '-c', $emf_file) or die "Cannot open EMF file <$emf_file>\n";
open(my $maf_fh, '|-', '/usr/bin/env gzip > ' . $maf_file) or die "Cannot open MAF file $maf_file\n";
